...
// Safe Date Formatter
// ─────────────────────────────────────────────────────────────────────────────
function formatDateSafe(dateInput: Date | string | number | undefined, formatStr: string): string {
    if (!dateInput) return '';
    try {
...
function LockedRulesOverlay({ connectionId }: { connectionId: string }) {
    return (
        <div className="relative" role="region" aria-label="Locked diagnostic results">
                ...
                <Table>
                    <TableHeader className="bg-slate-50/50">
                        <TableRow className="hover:bg-transparent border-slate-100">
                            <TableHead className="w-[50%] text-[10px] font-black uppercase tracking-widest text-slate-500 py-2 h-10 pl-6">Diagnostic Rule</TableHead>
                            <TableHead className="w-[18%] text-[10px] font-black uppercase tracking-widest text-slate-500 py-2 h-10">Status</TableHead>
                            <TableHead className="w-[16%] text-[10px] font-black uppercase tracking-widest text-slate-500 py-2 h-10 text-right">Exceptions</TableHead>
                            <TableHead className="w-[16%] text-[10px] font-black uppercase tracking-widest text-slate-500 py-2 h-10 text-right pr-6">Last Execution</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {PLACEHOLDER_ROWS.map((row, i) => (
                            <TableRow key={i} className="border-slate-100">
                                <TableCell className="w-[50%] py-3 pl-6">
                                ...
                                <TableCell className="w-[16%] text-right pr-6">
            ...
            {/* Lock overlay */}
            <div className="absolute inset-0 backdrop-blur-[3px] bg-white/70 flex flex-col items-center justify-center gap-6 rounded-xl">
                ...
                <div className="flex items-center justify-between p-4 border-b border-slate-50 bg-slate-50/20">
                    ...
                    {props.runAt && (
                        <span className="text-[10px] text-slate-400 font-mono">
                            Run: {formatDateSafe(props.runAt, 'MMM d, h:mm a')}
                        </span>
                    )}
                </div>
                {/* 
...
function UnlockedRulesTable({ checks, issues, connectionId }: RulesTableProps) {
    ...
    // Focus Mode: Hide passed rules, hydrated safely to prevent SSR mismatch
    ...
    // Pre-aggregate issue counts by ruleId for fast O(1) lookup
    const issueCountsByRule = useMemo(() => {
        const map = new Map<string, number>();
        for (const issue of issues || []) {
            if (issue.ruleId) {
                const current = map.get(issue.ruleId) || 0;
                map.set(issue.ruleId, current + (issue.entityCount || 0));
            }
        }
        return map;
    }, [issues]);
    ...
    const selectedIssues = useMemo(() => {
        ...
        return (issues || []).filter(i => i.ruleId === selectedRuleId);
    }, [issues, selectedRuleId]);
                                            ...
                                            <AccordionItem key={category} value={category} className="border-b border-slate-100 last:border-0">
                                                <AccordionTrigger className="px-4 py-3 hover:no-underline group">
                                                    <div className="flex items-center gap-3 w-full">
                                                        ...
                                                        <ChevronDown className="h-3.5 w-3.5 text-slate-400 group-data-[state=open]:rotate-180 transition-transform ml-auto" />
                                                    </div>
                                                </AccordionTrigger>
                                                    ...
                                                    <Table className="border-0">
                                                        <TableBody>
                                                            {categoryChecks.map((check) => {
                                                                ...
                                                                const issueCount = issueCountsByRule.get(check.ruleId) || 0;
                                                                return (
                                                                    <TableRow
                                                                        key={check.ruleId}
                                                                        ...
                                                                        tabIndex={0}
                                                                        role="button"
                                                                        aria-label={`View audit details for ${check.ruleName}`}
                                                                        onKeyDown={(e) => {
                                                                            if (e.key === 'Enter' || e.key === ' ') {
                                                                                e.preventDefault();
                                                                                setSelectedRuleId(check.ruleId);
                                                                            }
                                                                        }}
                                                                    >
                                                                        <TableCell className="w-[50%] py-2.5 pl-6">
                                                                            <div className="flex items-center gap-3">
                                                                                <div className={cn("w-1 h-8 rounded-full shrink-0", config.indicator)} />
                                                                        ...
                                                                        <TableCell className="w-[16%] py-2.5 text-right font-mono text-[12px] font-bold text-slate-600">
                                                                            {issueCount > 0 ? (
                                                                        ...
                                                                        <TableCell className="w-[16%] py-2.5 text-right pr-6">
                                                                            <div className="flex items-center justify-end gap-2">
                                                                                <span className="text-[10px] font-mono font-bold text-slate-400">{formatDateSafe(check.createdAt, 'MMM d')}</span>
