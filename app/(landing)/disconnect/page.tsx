    ...
    useEffect(() => {
        async function triggerCleanup() {
            try {
                ...
                // Slight delay for a smoother UI transition, preventing aggressive flashing
            ...
            <div className="relative w-full max-w-md overflow-hidden rounded-3xl bg-white/70 backdrop-blur-xl border border-white/40 p-8 shadow-2xl text-center">
                ...
                {isVerifying ? (
                    <div className="flex flex-col items-center justify-center space-y-6 py-8">
                        <div className="relative flex items-center justify-center w-20 h-20 bg-blue-50 rounded-2xl">
                            <Loader2 className="w-10 h-10 text-blue-600 animate-spin" />
                        </div>
                        <div className="space-y-2">
                            <h2 className="text-xl font-bold text-gray-900 tracking-tight">Verifying Disconnect...</h2>
                            <p className="text-sm font-medium text-gray-500">Securing your session and updating connection status.</p>
                        </div>
                    </div>
                ) : (
